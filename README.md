...
### 阴阳怪气类词条（典 / 孝 / 急 / 绷 等）的收录思路

这类词的特点：**单字、且本身是常用字**（经典、孝顺、着急、绷带），直接收录必然误伤。可行方案是两条腿：

1. **独立成句用边界正则**——要求该字前后都不能是汉字，只匹配"单独一个字（可带标点/表情）"的弹幕：
...
2. **固定搭配单独收**：`典中典`、`太典了`、`孝掉大牙`、`大孝子`、`急了急了`、`不会吧不会吧`、`你品你细品`、`你在教我做事`、`老阴阳人` 等。

实测（29849 条真实弹幕）：上述规则零误报——`经典`/`典礼`/`孝顺`/`着急`/`紧急`/`绷带` 都不会命中。
代价是只覆盖"独立成句 + 已知搭配"两种形态：像「一直吹一加」「有的人是不是眼睛有问题」这类
**语境型阴阳怪气**靠词表无法可靠识别，交给 AI 判定更合适。
